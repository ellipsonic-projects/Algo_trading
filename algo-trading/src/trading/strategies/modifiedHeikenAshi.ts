...
export type ModifiedHeikenAshiCandle = {
    ts: string
    open: number
    high: number
    low: number
 ...
 */
export function computeModifiedHeikenAshi(candles: Candle[]): ModifiedHeikenAshiCandle[] {
    if (candles.length === 0) return []

    const haCandles: ModifiedHeikenAshiCandle[] = []
...
export type ModifiedHeikenAshiTrend = 'BULLISH' | 'BEARISH' | 'NEUTRAL'

export type StrategySignal = {
    trend: ModifiedHeikenAshiTrend
    ema: number
    jma: number
    haClose: number
    isEntry: boolean
 ...
 */
export function analyzeModifiedHeikenAshiStrategy(
    haCandles: ModifiedHeikenAshiCandle[],
    emaPeriod: number = 20,
    jmaLength: number = 7
): StrategySignal {
    ...
    let trend: ModifiedHeikenAshiTrend = 'NEUTRAL'
